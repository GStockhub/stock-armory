...
def convert_gsheet_url(url):
    ...
    if "docs.google.com/spreadsheets/d/" in url and "export?format=csv" not in url:
        ...
        if match:
            ...
            gid = "0" if "gid=" not in url else url.split("gid=")[1].split("&")[0]
...
def safe_download(sid, fm_token=None):
    ...
    is_tw = sid_str[0].isdigit() if len(sid_str) > 0 else False
    
    if is_tw and fm_token and fm_token.strip() != "":
        try:
    ...
    try:
        syms = [f"{sid_str}.TW", f"{sid_str}.TWO"] if is_tw else [sid_str]
        session = get_yf_session()
        for sym in syms:
            df = yf.download(sym, period="3mo", progress=False, session=session)
