...
def run_sandbox_sim(sid, TWSE_NAME_MAP, fm_token=None):
    ...
    sig_a = (
        (df_bt["Volume"] > df_bt["Vol_MA5"] * 1.4) &
        (df_bt["K"] > 75) &
    ...
    for idx in signals_idx:
        ...
        if prev_close > 0 and entry_p > prev_close * 1.04:
            continue
        ...
        for _, row in future_data.iterrows():
            ...
            if not sold_half and curr_p >= tp_target:
                sold_half = True
            ...
            if sold_half:
                ret = 0.5 * ((tp_target - entry_p) / entry_p) + 0.5 * ((final_p - entry_p) / entry_p)
            else:
...
def level2_quant_engine(id_tuple, TWSE_IND_MAP, TWSE_NAME_MAP, MACRO_SCORE, fm_token=None):
    ...
    for sid in id_list:
        try:
            ...
            # 條件稍微放寬，避免太常全空
            if prev_close > 0 and ((open_now - prev_close) / prev_close * 100) > 6.0:
                continue
            if p_now < 10:
                continue
            if vol_now < 0.3:
            ...
            for idx in signals_idx:
                ...
                if prev_close_bt > 0 and entry_p > prev_close_bt * 1.04:
                    continue
            ...
            if len(sim_returns) < 5:
                ...
                win_rate = (np.array(sim_returns) > 0).mean() * 100
                avg_ret = np.array(sim_returns).mean() * 100
            ...
            intel_results.append({
                ...
                "停利價": round(p_now + 2.0 * atr_now, 2),
                "原始風險差額": round(raw_risk, 4),
                "戰術型態": tactic_label
            })
