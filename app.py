...
st.markdown("<p style='text-align: center;' class='text-sub'>—— 主動ETF ✕ 手機沙盤 ——</p>", unsafe_allow_html=True)
...
warroom_ui.render_reading_rule(st, COLORS)
...
with t_rank:
    ...
    warroom_ui.render_section_brief(
        st, COLORS,
        "🎯 個股游擊｜短線作戰桌",
        "這一區只回答：今天哪些股能短打、哪些只能觀察、哪些因流動性或過熱要排除。S/A/B 不是無腦買，仍需丟沙盤與開盤價確認。",
        [
            ("主攻", "S/A 級", "9:30 後仍強才打；跌破開盤價先不追。"),
            ("回踩", "B 級", "13:00 後看是否站穩 M5/M10。"),
            ("地雷", "低量排除", "20日均量與成交金額不足，不准進 S/A。"),
            ("決策", "沙盤優先", "清單只是候選，沙盤才是買前檢查。"),
        ],
        "已納入流動性分級與假放量警告，避免 4198 類低量股排到前面。"
    )
...
with t_etf:
    warroom_ui.render_section_brief(
        st, COLORS,
        "📈 ETF 主體倉｜資金主線雷達",
        "這一區回答：ETF 動能在哪、主動 ETF 經理人偏向哪些產業、共同重倉是否一致。資料分 A/B/C/D，但 A/B/C 都能協助判斷大方向。",
        [
            ("用途", "60% 主體倉", "偏 5～10 天短波段，不跟個股 S/A/B 混排。"),
            ("來源", "可信度分級", "官方完整 > 官方動態 > 第三方備援 > 未確認。"),
            ("沒事件", "看持股方向", "沒有調倉事件時，改看共同重倉與產業共識。"),
        ],
        "ETF 新聞與情報局只做背景驗證；實際買賣仍看 ETF 動能、乖離與沙盤。"
    )
...
with t_chip:
    warroom_ui.render_section_brief(
        st, COLORS,
        "📡 情報局｜新聞 × 盤面 × ETF 驗證",
        "這一區回答：今天市場為什麼這樣走、新聞主線是否被盤面承認、哪些題材只是有聲量但還沒驗證。新聞不直接變買賣訊號。",
        [
            ("新聞", "全自動", "GitHub Actions 每日產出 intel_daily.json。"),
            ("驗證", "盤面/ETF/法人", "新聞偏多但盤面不認帳，就降級看待。"),
            ("輸出", "三句話", "先看市場解釋，再看產業熱度與風險。"),
        ],
        "定位：新聞解釋背景，ETF/法人/技術面負責驗證，沙盤推演才負責決策。"
    )
...
with t_cmd:
    warroom_ui.render_section_brief(
        st, COLORS,
        "🏦 總司令部｜持股與紀律中樞",
        "這一區回答：目前持股有沒有破防、資金是否被卡住、AAR 是否出現重複錯誤。它不是選股區，是風控與紀律區。",
        [
            ("持股", "先處理風險", "破 M10、跌破硬底線、主題失效優先處理。"),
            ("AAR", "看行為", "追高、凹單、忘記賣一半會被紀錄。"),
            ("追蹤", "驗證系統", "用訊號追蹤室看 S/A/B 是否真的有效。"),
        ],
        "手機只看持股與沙盤；電腦才進總司令部做完整檢討。"
    )
...
with t_book:
    warroom_ui.render_section_brief(
        st, COLORS,
        "📖 兵工廠與軍史館｜本頁術語與戰法說明",
        "這一區保留獨立頁面，不塞進側邊欄。用途是查規則、查名詞、看歷史戰役；主畫面不被教學文字污染。",
        [
            ("快速版", "每日作戰卡", "手機與盤中只看這個。"),
            ("分頁教學", "依區域查", "個股、ETF、情報局、司令部各自有說明。"),
            ("軍史館", "版本紀錄", "保留系統演進與失敗教訓。"),
        ],
        "未來每個區域新增指標，都先補到這裡，不要全部塞在主畫面。"
    )
    quick_tab, context_tab, full_tab, hist_tab = st.tabs(["⚡ 快速版", "🧭 分區名詞", "📚 完整兵工廠", "🏛️ 軍史館"])
    with quick_tab:
        st.markdown(QUICK_MANUAL_TEXT, unsafe_allow_html=True)
    with context_tab:
        st.markdown(CONTEXT_MANUAL_TEXT, unsafe_allow_html=True)
    with full_tab:
