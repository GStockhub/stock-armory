...
if auth_status not in ["admin_auth", "guest_auth"]:
    ...
    with col2:
        ...
        if st.button("🔓 驗證並解鎖", use_container_width=True) or pwd:
            if pwd == ADMIN_PWD:
                st.session_state["v3_auth_token"] = "admin_auth"  # 標記為大將軍
                st.success("✅ 統帥確認：...正在為您開啟專屬戰情室...")
                time.sleep(1.2)
                st.rerun()
            elif pwd == GUEST_PWD:
                st.session_state["v3_auth_token"] = "guest_auth"  # 標記為友軍
...
[data-testid="stMarkdown"] { flex: 1; display: flex; flex-direction: column; }
...
/* 徽章列：即使無徽章也佔位 26px，確保下方對齊 */
.card-badges { flex-shrink: 0; min-height: 26px; display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 8px; align-items: center; }
/* 評分區固定不縮 */
.card-score { flex-shrink: 0; }
/* 底部資料區：吃掉剩餘空間，讓底部永遠對齊 */
.card-body { flex: 1; display: flex; flex-direction: column; justify-content: flex-end; width: 100%; }
...
.stock-title {
    margin: 0; font-size: clamp(14px, 1.1rem, 18px);
                    ...
                    for idx, (_, r) in enumerate(ui_s.iterrows()):
                        if idx < 3: 
                            with cols_s[idx]:
                                ...
                                card_html += f'<div class="card-header"><div style="margin-bottom: 8px; display: flex; align-items: flex-start; gap: 6px;"><span class="tier-badge badge-s" style="margin-top:3px;">🥇 S級</span><h3 class="stock-title" style="color: {COLORS["primary"]};">{r["名稱"]} ({r["代號"]})</h3></div></div>'
                                card_html += f'<div class="card-badges"></div>'
                                card_html += f'<p style="color: #A0A0A0; margin: 0 0 8px 0; font-size: 12px; flex-shrink:0;">{r["產業"]} | 投信連買 {r["連買"]} 天</p>'
                                card_html += f'<div class="card-score"><div style="background-color: {COLORS["bg"]}; padding: 10px; border-radius: 6px; margin-bottom: 10px; border-left: 3px solid {COLORS["green"]};">'
                                ...
                                card_html += f'<div style="color: {COLORS["text"]}; font-size: 12px; font-weight: bold; margin-top: 4px;">{r["戰術型態"]} | <span style="color:{COLORS["accent"]}">{r["生命週期"]}</span></div></div></div>'
                    ...
                    for idx, (_, r) in enumerate(ui_a.iterrows()):
                        if idx < 3: 
                            with cols_a[idx]:
                                ...
                                card_html += f'<div class="card-header"><div style="margin-bottom: 8px; display: flex; align-items: flex-start; gap: 6px;"><span class="tier-badge badge-a" style="margin-top:3px;">🥈 A級</span><h3 class="stock-title" style="color: {COLORS["accent"]};">{r["名稱"]} ({r["代號"]})</h3></div></div>'
                                card_html += f'<div class="card-badges"></div>'
                                card_html += f'<p style="color: #A0A0A0; margin: 0 0 8px 0; font-size: 12px; flex-shrink:0;">{r["產業"]} | 投信連買 {r["連買"]} 天</p>'
                                card_html += f'<div class="card-score"><div style="background-color: {COLORS["bg"]}; padding: 10px; border-radius: 6px; margin-bottom: 10px; border-left: 3px solid {COLORS["green"]};">'
                                card_html += f'<div class="info-row"><span class="info-label" style="font-weight:bold; color: {COLORS["text"]};">🎯 量化評分</span><span class="info-value" style="font-size: 16px; color: {COLORS["text"]}; font-weight:bold;">{r["Quant_Score"]} 分</span></div>'
